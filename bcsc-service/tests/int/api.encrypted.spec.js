...
jest.mock('axios', () => ({
    ...
    get: jest.fn((_url, _body) => { 
        return new Promise((resolve) => {
            ...
            resolve({ data: encryptedUserInfo });
        });
    })
}));
...
describe('api routes', () => {
    it('should return decrypted user info when get "/api/auth/:code" ', async (done) => {
        const code = "CODE";
        request(app)
            .get(`/api/auth/${code}`)
            .expect(decryptedUserInfo)
            .expect(200, done)
    })

    it('should return decrypted user info when get "/api/userinfo" with a token', async (done) => {
