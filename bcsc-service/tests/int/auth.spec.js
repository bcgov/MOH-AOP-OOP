...
describe("getInfo", () => {
    it('sends GET request for unencrypted data and returns the correct data', async () => {
        const token = "TOKEN";
    ...
    it('sends GET request for encrypted data and returns the correct decrypted data', async () => {
        const token = "TOKEN";
        ...
        res = { data: encryptedUserInfo };
        axios.get.mockImplementationOnce(() => Promise.resolve(res));
        await expect(getInfo(configJwe, token)).resolves.toEqual(decryptedUserInfo);
        expect(axios.get)
            .toHaveBeenCalledWith( configJwe.info_uri, {
                headers: {
