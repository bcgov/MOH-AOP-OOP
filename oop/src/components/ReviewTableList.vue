<template>
  <div :class="className">
    ...
    <ReviewTable
      :elements="yourInfoTableData"
      :background-color="tableBackgroundColor"
    />

    <div v-if="$store.state.form.accountType === 'AH'">
      <div class="row align-items-end mt-5">
        ...
        >
          <a
            href="javascript:void(0)"
            @click="navigateToAccountTypePage()"
            >Edit
            <font-awesome-icon icon="pencil-alt" />
          </a>
        </div>
...
export default {
  ...
  computed: {
    ...
    accountTypeTableData() {
      ...
      if (
        this.$store.state.form.personMoving === "DEP_ONLY" ||
        this.$store.state.form.isAllDependentsMoving === "N"
      ) {
        items.push({
    ...
    moveInfoTableData() {
      ...
      if (this.$store.state.form.country === "Canada") {
        ...
        if (this.$store.state.form.city !== null) {
          const cityLabel =
            this.$store.state.form.country === "United States" ? "City:" : "City, Province:";
          items.push({
            label: cityLabel,
            value: this.$store.state.form.city,
          });
        ...
        if (this.$store.state.form.zipCode !== null && this.$store.state.form.zipCode !== "") {
          const zipLabel =
            this.$store.state.form.country === "United States"
              ? "Zip code (optional):"
