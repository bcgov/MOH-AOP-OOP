...
describe("FileUploader component", () => {
  ...
  test("change event", done => {
    ...
    const imageContents = fs.readFileSync(
      "src/modules/common/components/file-uploader/test-files/sample.jpg",
      { encoding: "base64" }
    );
  ...
  test("checkImageExists(): false", () => {
    ...
    imageExists = wrapper.vm.checkImageExists(image, [
      new CommonImage("file content")
    ]);
    expect(imageExists).toBeFalsy();
  });
  ...
  describe("readPDF()", () => {
    ...
    beforeEach(() => {
      wrapper = mount(FileUploader, {});
      pdfContents = fs.readFileSync(
        "src/modules/common/components/file-uploader/test-files/sample.pdf",
        { encoding: "base64" }
      );
    ...
    test("readPDF()", done => {
      wrapper.vm.readPDF(pdfFile, new CommonImageScaleFactorsImpl(1, 1), () => {
        done();
      });
    });
  ...
  test("getErrorMessage()", () => {
    const wrapper = mount(FileUploader, {});
    expect(wrapper.vm.getErrorMessage(CommonImageError.WrongType)).toBe(
      "Wrong file type."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.TooSmall)).toBe(
      "File too small."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.TooBig)).toBe(
      "File too large."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.AlreadyExists)).toBe(
      "File already exists."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.Unknown)).toBe(
      "Unknown error."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.CannotOpen)).toBe(
      "Cannot open file."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.PDFnotSupported)).toBe(
      "This PDF file is not supported."
    );
    expect(wrapper.vm.getErrorMessage(CommonImageError.CannotOpenPDF)).toBe(
  ...
  test("checkImageDimensions()", () => {
    const wrapper = mount(FileUploader, {});
    expect(
      wrapper.vm.checkImageDimensions({
        naturalWidth: 10,
        naturalHeight: 10
      })
    ).toBeTruthy();
